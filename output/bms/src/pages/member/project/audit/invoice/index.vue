 ...
 * @LastEditTime : 2026-03-17 16:59:13
 * @Description  : 项目-业务审核-开票审核 /member/project/audit/invoice
-->
<template>
  <view>
    <NavBar></NavBar>
    <view class="business-access">
      ...
      <view class="content">
        <uni-list v-if="vm.fgaiAppList && vm.fgaiAppList.length">
          <view v-for="(item, index) in vm.fgaiAppList" :key="item.refcode" class="list-item">
            <uni-list-item>
...
import apiInvoice from '@/api/member/project/audit/api.invoice'
...
const viewDetail = (refcode: number) => {
  router.push(`/member/project/audit/invoice/audit/${encodeBase64(refcode)}`)
}
/**
 * @Author: Charon.Lin
