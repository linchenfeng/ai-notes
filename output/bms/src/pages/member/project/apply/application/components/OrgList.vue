 ...
 * @LastEditors  : ran.zhu
 * @LastEditTime : 2025-09-23 17:58:36
 * @Description  : 选择买方
-->
<template>
...
// 定义一个基础接口，要求必须包含refcode字段（用于点击匹配）
interface BaseItem {
  refcode: string
}
// 通过类型别名接收父组件传递的类型
type ItemType = BaseItem & Record<string, any>
// 定义emits类型
...
type PropsType = {
  ...
  searchFun: (val: string) => Promise<any>
}
...
const clickItem = (val: ItemType) => {
  const selectItem = selectCountryList.data.find(item => item[props.idKey] === val[props.idKey])
  emits('click-fun', selectItem)
}
const back = () => {
  emits('back')
}
...
onMounted(() => {
  selectCountryList.data = Array.isArray(props.data) ? props.data : []
})
</script>
