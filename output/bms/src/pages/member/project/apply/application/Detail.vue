<!--
 * @Author       : Charon.Lin
 * @Date         : 2025-09-23 14:22:08
 * @LastEditors  : aolin.qu
 * @LastEditTime : 2026-03-19 10:47:30
 * @Description  : 填报申请书详情页
-->
<template>
  <NavBar>
    ...
    <view class="application-application-apply">
      <u-form label-width="35%">
        ...
        <ty-info-card title="买方信息">
          <view v-if="showEditFlag">
            <view v-for="(item, i) in vm.submitInfo.buyerList" :key="i" class="buyer-swipe" @click="addBuyerInfo(item)">
        ...
        <ty-info-card title="缴纳征信费" class="visible-card">
          <view class="internation-info-group">
            <u-cell-group>
        ...
        <ty-info-card v-if="isNotEmpty(vm.submitInfo.fintAppM.billFlag)" title="开票信息" class="visible-card">
...
const addBuyerInfo = item => {
  ...
  router.push(`/member/project/apply/application/buyerInfoDetail/${encodeBase64(item.refcode)}`)
}
/**
 * @Author: LongCan.Yang
