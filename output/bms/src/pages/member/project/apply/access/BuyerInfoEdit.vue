 ...
 * @LastEditTime : 2026-02-03 10:57:34
 * @Description  : 项目-业务申请-补充买方信息 /member/project/apply/access/buyerInfo
-->
<template>
  <NavBar />
    ...
    <u-form label-width="43%">
      <ty-info-card :show-line="false" :title="productType.includes('国内电商') && bizScene === '1' ? '贸易信息' : ''">
        <u-form-item label="买方名称" required>
          <view
            :class="productType === '国内电商' && bizScene === '1' ? 'unFocus' : ''"
            @click="productType === '国内电商' && bizScene === '1' && selectName()"
          >
            <u-input v-model="vm.buyerItem.counterpartyName" placeholder="请选择买方名称" />
          </view>

          <template v-if="vm.buyerList[0].length || (productType === '国内电商' && bizScene === '1')" #right>
        ...
        <ShopFinanceInfo
          v-if="productType === '国内电商' && bizScene === '1'"
          ref="shopFinanceRef"
          v-model:form-str="vm.buyerItem.storeInformation"
          :ccyid="vm.buyerItem.ccyid"
        />
        <u-form-item label="备注说明"></u-form-item>
        <u-textarea
          v-model="vm.buyerItem.remark"
          :confirm-type="null"
          maxlength="300"
...
import ShopFinanceInfo from './components/shopFinanceInfo/index.vue'
import { useShopFinance } from './components/shopFinanceInfo/useShopFinance'
...
const { buyerNameOptions } = useShopFinance() //国内电商买方列表
let showPicker = ref(false) // 买方列表弹窗
...
// 1. 定义子组件ref类型（解决TS类型提示）
type ShopFinanceInfoInstance = {
  validateForm: ReturnType<typeof useShopFinance>['validateForm']
  getSubmitStr: ReturnType<typeof useShopFinance>['getSubmitStr']
}
// 2. 初始化子组件ref
const shopFinanceRef = ref<ShopFinanceInfoInstance>()
/**
 * @Author: Charon.Lin
...
const selectName = () => {
  //国内电商买方名称选项固定
  vm.nameColumns = productType.value === '国内电商' && bizScene.value === '1' ? [buyerNameOptions] : vm.buyerList
...
const doConfirm = value => {
  ...
  //国内电商不带出国家
  if (!(productType.value.includes('国内电商') && bizScene.value === '1')) {
    vm.buyerItem.ctyid = value.value[0].orgctyid
    vm.buyerItem.ctyDesc = value.value[0].ctyDesc
  }
...
const selectPicker = (type: string) => {
  ...
  if (type === '1') {
    ...
    const defalultIndex = vm.columns[0].findIndex(item => item.key === vm.buyerItem.tenorStartType)
    defaultIndex.value = [defalultIndex === -1 ? 0 : defalultIndex]
  } else if (type === '2') {
    vm.columns = [getConstant('applyCcyid').concat([{ key: 'qt', value: '其他' }])]
    const defalultIndex = vm.columns[0].findIndex(item => item.key === vm.buyerItem.ccyid)
    defaultIndex.value = [defalultIndex === -1 ? 0 : defalultIndex]
  } else {
    vm.columns = [getConstant('waybillType')]
    const defalultIndex = vm.columns[0].findIndex(item => item.key === vm.buyerItem.waybillVerifyType)
    defaultIndex.value = [defalultIndex === -1 ? 0 : defalultIndex]
  }
...
const onSubmit = () => {
  ...
  const baseFieldsValid = !(
    ...
    (productType.value.includes('物流') &&
      (isEmpty(vm.buyerItem.logisticsSystemFlag) || isEmpty(vm.buyerItem.waybillVerifyType) || isEmpty(vm.buyerItem.publicBiddingFlag))) ||
    isEmpty(vm.buyerItem.tenorStartDesc) ||
    // eslint-disable-next-line no-extra-parens
  ...
  if (productType.value.includes('国内电商') && bizScene.value === '1') {
    if (shopFinanceRef.value) {
      vm.buyerItem.storeInformation = shopFinanceRef.value.getSubmitStr()
      shopFinanceValid = shopFinanceRef.value.validateForm()
    }
  }
  filloutFlag = baseFieldsValid && shopFinanceValid
  vm.buyerItem.supplementStatus = filloutFlag ? '1' : '0'
  if (index === -1) {
    //买方列表第一个插入
    // 校验buyerItem里的买方名称在fintAplDtcVoList是否存在
    if (vm.fintAplDtcVoList.some(item => item.counterpartyName === vm.buyerItem.counterpartyName)) {
      showToast('买方名称不能重复')
      return
    }
    vm.fintAplDtcVoList.push(vm.buyerItem)
  } else {
    const isDuplicate = vm.fintAplDtcVoList.some((item, idx) => {
      // 条件1：排除当前编辑的项（通过索引判断）
      // 条件2：比较买方名称（忽略前后空格，不区分大小写）
      return idx !== index && item.counterpartyName === vm.buyerItem.counterpartyName
    })
    if (isDuplicate) {
      showToast('买方名称不能重复')
      return
...
onMounted(() => {
  ...
  if (productType.value.includes('物流') && isEmpty(vm.buyerItem.productName)) {
    vm.buyerItem.productName = '物流运输服务'
  }
  if (productType.value.includes('物流') && isEmpty(vm.buyerItem.tenorStartType)) {
    vm.buyerItem.tenorStartType = '2'
    vm.buyerItem.tenorStartDesc = '发票日'
  }
  if (productType.value.includes('物流') && isEmpty(vm.buyerItem.ccyid)) {
  ...
  // 国内电商初始化买方所在国为中国
  if (productType.value.includes('国内电商') && bizScene.value === '1' && isEmpty(vm.buyerItem.ctyid)) {
    vm.buyerItem.ctyid = 'CHN'
    vm.buyerItem.ctyDesc = '中国'
  }
  doShareInfo()
