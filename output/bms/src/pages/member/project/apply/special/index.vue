 ...
 * @LastEditTime : 2025-12-12 10:33:34
 * @Description  : 项目-业务申请-特批申请列表 /member/project/apply/special
-->
<template>
  <view>
    <NavBar>
    ...
    <view class="business-access">
      ...
      <view class="content">
        <uni-list v-if="vm.saaList && vm.saaList.length">
          <uni-list-item v-for="item in vm.saaList" :key="item.refcode" class="list-item" @tap="viewDetail(item.refcode)">
            <u-cell-group class="listData" :border="false">
              ...
              <!-- 已退回的数据展示申请编号 -->
              <u-cell v-if="item.applyStatus !== '0'" :stop="false" title="申请编号" :value="item.financingNo" :border="false" />
...
import { SpecialItem } from '@/interfaces/member/project/apply/specialApproval'
...
const getSpecialAppCount = () => {
  ...
  apiSpecialApproval.getSpecialAppCount(data).then(res => {
    vm.tabsList = [
      {
        name: '待提交' + '(' + res.data.submitCount + ')'
      },
      {
        name: '已提交' + '(' + res.data.pendingCount + ')'
      }
    ]
  })
}
...
const findSpecialAppPage = () => {
  apiSpecialApproval.findSpecialAppPage(search).then(res => {
...
// 获取申请类型描述
const getDesc = (item: SpecialItem) => {
  if (item.bizScene === '1') {
...
const viewDetail = (refcode: number) => {
  router.push(`/member/project/apply/special/apply/${encodeBase64(refcode)}/${encodeBase64(search.data.applyStatus)}`)
}
/**
 * @Author: Charon.Lin
 ...
 */
const startApply = () => {
  router.push('/member/project/apply/special/apply')
}
/**
 * @Author: LongCan.Yang
