 ...
 * @LastEditTime : 2025-11-20 10:10:38
 * @Description  : 项目-业务申请-申请协销/征信 /member/project/apply/assist
-->
<template>
  <view>
    <NavBar></NavBar>
    <view class="assist">
      ...
      <view class="content">
        <uni-list v-if="vm.applyAssistList && vm.applyAssistList.length">
          <uni-list-item v-for="item in vm.applyAssistList" :key="item.refcode" class="list-item" @tap="viewDetail(item.aplRefcode, item.refcode)">
                  ...
                  <u-col :span="4">
                    <view class="status-box">
                      <view class="status" :class="`status${item.applyStatus}`">
                        {{ getValueByKey(item.applyStatus, 'assApplyStatus') }}
                      </view>
                    </view>
                  </u-col>
                </u-row>
...
import { ApplyAssistList, Search } from '@//interfaces/member/project/apply/assist'
...
let applyAssistList: ApplyAssistList[] = []
 ...
 * @description: 获取笔数
 */
const getApplyAssistCount = () => {
  const data = {
    orgname: search.data.orgname
  }
  apiAssist.getApplyAssistCount(data).then(res => {
    vm.tabsList = [
      {
        name: '待申请' + '(' + res.data.applyCount + ')'
      },
      {
        name: '待提交' + '(' + res.data.waitSubmitCount + ')'
      },
      {
        name: '已申请' + '(' + res.data.submitCount + ')'
      }
    ]
  })
}
 ...
 * @description: 获取申请协销/征信列表
 */
const findApplyAssistPage = () => {
  apiAssist.findApplyAssistPage(search).then(res => {
    totalCount.value = res.totalCount as number
    if (res.data.applyAssistList && res.data.applyAssistList.length) {
      vm.applyAssistList.push(...res.data.applyAssistList)
    }
    if (totalCount.value === vm.applyAssistList.length && totalCount.value > 0) {
      finished.value = true
    }
    getApplyAssistCount()
  })
...
const selectTab = item => {
  ...
  switch (item.index) {
    case 0:
      search.data.queryStatus = '1'
      break
    case 1:
      search.data.queryStatus = '2'
      break
    case 2:
      search.data.queryStatus = '3'
      break
    default:
      break
  }
 ...
 * @description: 跳转至申请协销/征信
 */
const viewDetail = (aplRefcode, refcode) => {
  let path = ''
  if ([0].includes(tabIndex.value)) {
    path = `/member/project/apply/assist/apply/${encodeBase64(1)}/${encodeBase64(aplRefcode)}/${encodeBase64(aplRefcode)}`
  } else if ([1].includes(tabIndex.value)) {
    path = `/member/project/apply/assist/apply/${encodeBase64(0)}/${encodeBase64(aplRefcode)}/${encodeBase64(refcode)}`
  } else {
    // 已删除 已退回跳到申请页
    path = `/member/project/apply/assist/detail/${encodeBase64(refcode)}`
  }
  router.push(path)
}
/**
