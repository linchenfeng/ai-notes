<!--
 * @Author       : Charon.Lin
 * @Date         : 2025-11-27 18:24:36
 * @LastEditors  : Charon.Lin
 * @LastEditTime : 2025-12-10 09:23:30
 * @Description  : 项目-业务申请-申请融资 /member/project/apply/loanApply
-->
<template>
  <view>
    <NavBar>
    ...
    <view class="business-access">
      ...
      <view class="content">
        <uni-list v-if="vm.fintLoaList && vm.fintLoaList.length">
          <uni-list-item v-for="item in vm.fintLoaList" :key="item.refcode" class="list-item" @tap="viewDetail(item.refcode, item.loanStatus)">
            <u-cell-group class="listData" :border="false">
              ...
              <u-cell :stop="false" title="授信产品" :value="getValueByKey(item.creditProducts as string,'fintCreditPro')" :border="false" />
...
import { FintLoaList, Search } from '@/interfaces/member/project/apply/loanApply'
...
const getApplyCount = () => {
  ...
  apiLoanApply.getApplyCount(data).then(res => {
    vm.tabsList = [
      {
        name: '待提交' + '(' + res.data.saveCount + ')'
      },
      {
        name: '已提交' + '(' + res.data.submitCount + ')'
      }
    ]
  })
}
 ...
 */
const findApplyList = () => {
  apiLoanApply.findApplyList(search).then(res => {
 ...
 * @description: 查看详情
 */
const viewDetail = (refcode: number, loanStatus: string) => {
  //已保存已撤回已退回跳申请页
  if (['0', '2', '4'].includes(loanStatus)) {
    router.push(`/member/project/apply/loanApply/apply/${encodeBase64(refcode)}`)
  } else {
    router.push(`/member/project/apply/loanApply/detail/${encodeBase64(refcode)}`)
  }
}
/**
 * @Author: Charon.Lin
 ...
 */
const startApply = () => {
  router.push('/member/project/apply/loanApply/apply')
}
/**
 * @Author: LongCan.Yang
