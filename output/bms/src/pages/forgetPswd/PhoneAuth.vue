<!--
 * @Author       : jacky.cai
 * @Date         : 2023-12-26 11:48:53
 * @LastEditors  : Charon.Lin
 * @LastEditTime : 2025-03-31 16:55:59
 * @Description  : 忘记密码-手机验证
-->
<template>
  <NavBar />
  <u-form>
    <u-input v-model="data.mobilephone" disabled border="bottom" maxlength="20" name="工号" placeholder="请输入工号" />
    <u-input
      v-model="data.code"
      ...
      maxlength="20"
      name="验证码"
      placeholder="请输入获取的短信验证码"
    >
      <template #suffix>
        <u-code ref="uCodeRef" unique-key="phone" start-text="获取验证码" @change="codeChange"></u-code>
...
let data = reactive({
  ...
  mobilephone: decodeBase64(String(route.value.params.mobilephone)),
  code: '',
  uuid: '',
  pswd: '',
  pswdAgain: ''
...
const getCode = (): void => {
  ...
  apiCommon.doSendPhoneCode(rData).then(
    res => {
      loading.value = false
...
const onSubmit = (): void => {
  ...
  validatePassword(data.pswd).then(result => {
    ...
    if (result) {
      ...
      apiCommon.doCheckPhoneVerificationCode(rData).then(
        res => {
          loading.value = false
          doResetPassword()
        },
