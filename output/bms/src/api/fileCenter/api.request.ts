 ...
 * @LastEditTime : 2025-10-31 10:46:54
 * @Description  : API 请求基类，请求文件中心
 */
 ...
 * @param {object} postData // 请求附带参数
 * @param {boolean} needToken // 是否需要拼接token,签约信息收集模块不拼接
 * @param {boolean} showLoadingFlag // 是否开启loading
 * @param {boolean} showTip // 是否展示接口异常信息
...
export async function request<T>(
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      uni.request({
        url: systemConfig.MINIO_URL + url,
        method,
...
export function download(url: string) {
  const { fileCenterToken } = storeToRefs(useFileCenterTokenStore())
  const { systemConfig } = useSystemConfigstore()
  let downloadUrl = systemConfig.MINIO_URL + url
  if (url.includes('?')) {
    downloadUrl += `&access_token=${fileCenterToken.value}`
  } else {
    downloadUrl += `?access_token=${fileCenterToken.value}`
  }
  return new Promise(function (resolve, reject) {
