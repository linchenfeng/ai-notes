<template>
  <view :class="defClass" :style="defStyle" v-if="routerViewIsRender">__dynamicImportComponent__</view>
</template>

<script>
import { defineComponent, computed, unref, watch } from 'vue'

import { routerAbstractRouteskey, viewDepthKey, instanceFindParentId, checkParentInstance } from './index.mjs'
  ...
  computed: {
    ...
    includeCom() {
      if (this.currentRoute) {
        return this.currentRoute.matched
          .map(it => {
